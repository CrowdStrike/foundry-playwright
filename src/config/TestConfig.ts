...
export class TestConfig {
  ...
  private constructor() {
    // Environment detection (must be set before timeout defaults reference isCI)
    ...
    // Core URLs (strip trailing slash for consistent URL construction)
    this.falconBaseUrl = (process.env.FALCON_BASE_URL || 'https://falcon.us-2.crowdstrike.com').replace(/\/+$/, '');
  ...
  public getPlaywrightTimeouts() {
    return {
      ...
      actionTimeout: this.isCI ? 15000 : 10000,
    };
  }
  ...
  public logSummary(): void {
    if (this.isCI) {
      console.log(`E2E Test Config: CI | ${this.appName}`);
    } else {
...
// This lets consumers import { defineFoundryConfig } without needing env vars set.
let _configInstance: TestConfig | undefined;

export const config: TestConfig = new Proxy({} as TestConfig, {
  get(_target, prop, receiver) {
    if (!_configInstance) {
      _configInstance = TestConfig.getInstance();
    }
    return Reflect.get(_configInstance, prop, receiver);
  },
});
