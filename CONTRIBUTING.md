...
- To provide Playwright E2E test infrastructure for Falcon Foundry apps
...
Good:
> AppCatalogPage was failing when apps had configuration screens because `installApp()` navigated away before setup completed. This adds an optional `configureSettings` callback so consuming apps can inject their config logic between install and navigation. The callback receives the page instance and runs inside the existing retry/wait logic, eliminating the race condition.

Bad:
> ## Summary
> This PR introduces several important updates to the AppCatalogPage, enhancing reliability and improving the overall developer experience.
>
> ## Changes
> - Updated AppCatalogPage.ts
> - Updated README.md
> - Added tests
>
> ## Test plan
...
AI coding assistants are welcome for developing contributions to this project. The bar for pull requests is the same regardless of how code was written. Pay extra attention to the [PR title and description style](#pr-title-and-description-style) section since AI tools tend to generate verbose, formulaic PR descriptions that don't match the style we're looking for.
